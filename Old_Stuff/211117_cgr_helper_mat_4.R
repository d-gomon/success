...
cgr_helper_mat_4 <- function(data, ctimes, coxphmod, cbaseh, ncores, displaypb = FALSE){
  #!
  ...
  if(ncores == "auto"){
    ncores <- max(1,real_cores -1)
  } else if(!missing(ncores)){
    if(ncores > real_cores){
      warning(paste0("More cores requested (", ncores ,") than detected (", real_cores,") by R. \n Proceed at own risk."))
    }
  }
  cl <- makeCluster(ncores)
  #progress bar doesnt work with parallelised computation
  displaypb <- FALSE
  ...
  fin <- parSapply(cl, ctimes, maxoverj)
  stopCluster(cl)
  if(displaypb){
    close(pb)
  }
