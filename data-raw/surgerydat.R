...
set.seed(01041996)
surgerydat <- data.frame(matrix(ncol=9,nrow=0, dimnames=list(NULL, c("entrytime", "survtime", "censorid", "unit", "expmu", "psival", "age", "sex", "BMI"))))
#censor some observations?
#i = hospital number
#j = value of psi
#k = exp(mu) >1 or not, sample from normal with high

#So we have 15 hospitals with mu sampled from norm(mean = log(0.5), sd = 0.25),
#15 hospitals with mu sampled from norm(mean = log(1), sd = 0.25) and 15 hospitals with norm(mean = log(2), sd = 0.25)
#They are divided into 3 groups of 5 hospitals with psi = 0.5, 1, 1.5
psivals <- c(0.5, 1, 1.5)
mumean <- c(log(1), log(1), log(1))
s <- 1
for(k in 1:3){
  for(j in 1:3){
    for(i in 1:5){
        arrivtimes <- round(gen_arriv_times(psi = psivals[j], t = 2*365))
        n <- length(arrivtimes)
        ...
        coxmodt <- list(formula = formula("~ age + BMI + sex"),
                        coefficients = c(age = 0.003, BMI = 0.02, sexmale = 0.2))
        mu = rnorm(1, mean = mumean[k], sd = 0.4)
